...
//    @Scheduled(cron = "1 * * * * ?")
    public void putUnreviewPatientsToRedis() {
        try {
            ...
            List<Patient> result = specialPatientRepository.putSpecialPatientToRedis(allSpPatients.toArray(new Patient[allSpPatients.size()]));
...
//    @Scheduled(cron = "1 * * * * ?")
    ...
    public void sendUnreviewSpecialPatientsListNotificationToFront() {
        try {
            ...
            amqpTemplate.convertAndSend("systemTempNotificationQueue", "当月还有" + currentUnreviewPetients.size() + "人需要复查，请提醒他们。");
        } catch (Exception e) {
